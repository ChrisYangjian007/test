<style>
    ...
    #updateScheduleTable td {
        font-size: 15px;
    }
</style>
<form id="" style="margin: 1px">
    <table class="form" id="updateScheduleTable">
        <thead>
...
<script>
    ...
    $(document).ready(function () {
        Loading(true, "正在加载", "#updateScheduleModal");
        $.post('${request.contextPath}/Home/getMonthSchedule.json', function (res) {
            let resObj = res.obj;
            if (res.success) {
                for (let i = 0; i < 6; i++) {
                    let arr = resObj[i].productSpec.split(',');
                    $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(0)").find("input").val(resObj[i].scheduleId);
                    $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(2)").find("select").val(arr).trigger("change");
                    $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(3)").find("input").val(resObj[i].amount);
                    $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(4)").find("option:selected").val(resObj[i].unitId);
                    $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(4)").find("option:selected").text(resObj[i].unitName);

                }
                Loading(false, "", "#updateScheduleModal");
            } else {
                Loading(false, "", "#updateScheduleModal");
    ...
    $("#updateScheduleButton").unbind("click");
    $("#updateScheduleButton").click(function () {
        Loading(true, "正在提交", "#updateScheduleModal");
        let allData = new Array();
        let length = $("#updateScheduleTable tbody>tr").length;
        for (let i = 0; i < length; i++) {
            ...
            data.scheduleId = $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(0)").find("input").val();
            data.productTypeId = $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(1)").find("input").val();
            data.productType = $.trim($("#updateScheduleTable tbody>tr").eq(i).find("td:eq(1)").text());
            let arr = $.trim($("#updateScheduleTable tbody>tr").eq(i).find("td:eq(2)").find("select").val());
            data.productSpec = arr.toString();
            data.amount = $.trim($("#updateScheduleTable tbody>tr").eq(i).find("td:eq(3)").find("input").val());
            data.unitId = $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(4)").find("option:selected").val();
            data.unitName = $("#updateScheduleTable tbody>tr").eq(i).find("td:eq(4)").find("option:selected").text();
            allData.push(data);
        }
        ...
        $.ajax({
            type: 'post',
            url: '${request.contextPath}/Home/updateSchedule.json',
