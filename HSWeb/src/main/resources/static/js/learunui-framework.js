/**
 * jQuery LeaRunUI 4.1
 *上海信息技术有限公司 Copyright © Wind 2014
 */
//样式
$(function () {
    readyIndex();
...
function errorGoHome() {
    AllcloseTab();
    Loading(true,"正在返回首页。。。");
    window.top.location= RootPath()+"/login.htm";
}

function openPostWindow(url,name1,data1){
    ...
    hideInput1.name=name1; //后台要接受这个参数来取值
 ...
 * 涛涛不要给我删掉，打印箱码专用
 * */
function openPostWindowAndSysLog(url,name1,data1,resourceId){
    var tempForm = document.createElement("form");
    tempForm.id = "tempForm1"
    tempForm.method = "post"
    tempForm.action = url
    tempForm.target="_blank" //打开新页面
    var hideInput1 = document.createElement("input")
        ,sysLog = document.createElement("input")
    hideInput1.type = "hidden"
    hideInput1.name=name1 //后台要接受这个参数来取值
    hideInput1.value = data1
    tempForm.appendChild(hideInput1)
    sysLog.type="hidden"
    sysLog.name="resourceId"
    sysLog.value=resourceId
    tempForm.appendChild(sysLog)
    if(document.all){
        tempForm.attachEvent("onsubmit",function(){});        //IE
    }else{
        var subObj = tempForm.addEventListener("submit",function(){},false);    //firefox
...
function staticImages() {
    return "http://s.xiaoqin.com.cn/static";
}
function staticCss() {
    return "http://s.xiaoqin.com.cn/static";
}
...
function Loading(bool,text,id) {
    ...
    html = '<div class="loading-Block">' +
        '<img src="'+staticImages()+'/images/loading.gif" style="vertical-align: middle;" >' +
        '<span>&nbsp;&nbsp;' + (text ? text: '正在拼了命为您加载…') + '</span>' +
        '</div>';
...
请求Ajax 带返回值
*/
function getAsyncAjax(url, postData, callBack) {
    $.ajax({
        type: 'post',
        dataType: "text",
        url: RootPath() + url,
    ...
    $(element).find('input,select,textarea').each(function (r) {
        ...
        switch (type) {
            ...
            default:
                ...
                //}
                value = value.replace("&nbsp;", '')
                reVal += '"' + id + '"' + ':' + '"' + $.trim(value).replace(/\"/g, "'").replace(/\n/g, '<br />') + '",'
                break;
        }
    });
...
function SetWebControls(data) {
    for (var key in data) {
        var id = $('#' + key);
        var value = $.trim(data[key]).replace("&nbsp;", "").replace(/<br\s*\/*>/gi, "\n");
        var type = id.attr('type');
        var m_class = id.attr('class');
        switch (type) {
            case "checkbox":
                if (value == 1) {
...
function getDataDictionary(Code) {
    this.url = "/CommonModule/DataDictionary/BinDingItemsJson";
    this.key = "Code";
    this.value = "CName";
    this.PostParmar = { Code: Code };
}

function getData(url, key, value, PostParmar) {
    this.url = url;
    this.key = key;
    this.value = value;
    this.PostParmar = PostParmar;
}
...
var formattingData = {
    ...
    Nature: new getDataDictionary("DepartmentNature"),
    Category: new getDataDictionary("CompanyCategory"),
    CompanyNature: new getDataDictionary("CompanyNature"),
    RoleCategory: new getDataDictionary("RoleCategory"),
    Grade: new getDataDictionary("Grade"),
    WarehouseID: new getData("/ZSModule/Warehouse/BinDingItemsJson", "ID", "CName", {}),
};
/*
自动给控件赋值、对Lable
*/
function SetWebLable(data) {
    ...
    //查看详情时
    $("[formattingData]").each(function () {
        var formattingDataName = $(this).attr("formattingData");
        var formattingDataobj = window.formattingData[formattingDataName]

        if (formattingDataobj) {
            if (formattingDataobj.url) {
                var $labelobj = $(this);

                (function (formattingDataName, formattingDataobj) {
                    $.post(formattingDataobj.url, formattingDataobj.PostParmar, function (formdatas) {
                        formdatas = JSON.parse(formdatas);
                        var value = data[$labelobj.attr("id")];
                        for (var i = 0; i < formdatas.length; i++) {
                            if (formdatas[i][formattingDataobj["key"]] == value) {
                                $labelobj.val(formdatas[i][formattingDataobj["value"]]);
                                $labelobj.html(formdatas[i][formattingDataobj["value"]]);
                                return;
                            }
                        }
                    })
                }(formattingDataName, formattingDataobj))
            } else if (formattingDataobj.datas) {
                var valueId = data[$(this).attr("id")];
                var value = formattingDataobj.datas[valueId] || "未知";
                $(this).val(value); $(this).html(value);
            }
        }
    })
}
        ...
        $(tableId + " tbody tr").each(function () {
            ...
            $(this).find('td').find('input,select,textarea').each(function () {
                ...
                if ($("#" + pk_id).attr('type') == "checkbox") {
                    ...
                    pk_value = $("#" + pk_id).val().replace("==请选择==", "");
                    //  pk_value = $("#" + pk_id).val() ? $("#" + pk_id).val().replace("==请选择==", "") : "";
                }
                var array = new Array();
...
function CheckedAvailabe(id, checkvalue) {
    var idArray = id.toString().split(',');
    for (i = 0; i < idArray.length; i++) {
        if (idArray[i] != checkvalue) {
            return false;
        }
    }
    return true;
...
function LoadrightMenu(element) {
    ...
    $(element).bind("contextmenu", function (e) {
        ...
        oMenu.show();
        oMenu.css('top', e.clientY + "px");
        oMenu.css('left', e.clientX + "px");
        //最大显示范围
...
function comboBoxTree(Objkey, height) {
    ...
    $("#" + comboBoxTree).css("left", Y).css("top", X + $("#" + Objkey).height() + 2).css("z-index", "400").slideDown(100);
    //任意键关闭
    document.onclick = function (e) {
        var e = e ? e : window.event;
        var tar = e.srcElement || e.target;
...
function SetUrl(url, parm) {
    confirmDialog("提示", "注：您确定要执行此操作？", function (r) {
        if (r) {
            Loading(true, "正在执行数据...");
            AjaxJson(url, parm, function (data) {
                tipDialog(data.Message, 3, data.Code);
...
function BindDropItem(ControlId, Code, Memo) {
    ...
    getAjax('/CommonModule/DataDictionary/BinDingItemsJson', { Code: Code }, function (data) {
        ...
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].Code).html(itemjson[i].CName));
        });
        Loading(false);
    });
}
...
///*获取数据字典
//*/
//function BindDropItem(Code, manageData) {
//    getAjax('/CommonModule/DataDictionary/BinDingItemsJson', { Code: Code }, function (data) {
//        manageData(data);
//    });
//}
...
Memo:默认显示
*/
function BindDropItemByUrl(ControlId, Post, Memo, Url) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value=''>" + Memo + "</option>");
    }
    getAjax(Url, Post, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].ID).html(itemjson[i].CName));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDropItemByData(ControlId, Post, Memo, Url, itemjson) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value=''>" + Memo + "</option>");
    }
...
function JsonBindDrop(ControlId, Memo, DataJson) {
    ...
    $.each(DataJson, function (i) {
        $(ControlId).append($("<option></option>").val(DataJson[i].Code).html(DataJson[i].CName));
    });
}
/*
验证对象值不能重复
...
//加载权限按钮
function PartialButton(staticImg) {
    ...
    AjaxJson("/Utility/LoadButton", {}, function (Data) {
        ...
        $.each(Data, function (i) {
            if (Data[i].ParentID == 0 && Data[i].Category == '1') {
                var toolbar_Children_List = Toolbar_Children(Data[i].ID, Data)
                if (toolbar_Children_Count > 0) {
                    toolbar_html += "<a id=\"" + Data[i].Code + "\" class=\"tools_btn dropdown\">";
                    toolbar_html += "<div style=\"float: left;\"><div class=\"icon\"><img src=\""+staticImg+"/images/Icon16/" + Data[i].Icon + "\" /></div><div class=\"text\">" + Data[i].CName + "</div></div>";
                    toolbar_html += "<div class=\"dropdown-icon\"><img src=\""+staticImg+"/images/dropdown-icon.png\" /></div>";
                    ...
                    toolbar_html += "<a id=\"" + Data[i].Code + "\" " + title + " onclick=\"" + Data[i].JsEvent + "\" class=\"tools_btn\"><span><b style=\"background: url('"+staticImg+"/images/Icon16/" + Data[i].Icon + "') 50% 4px no-repeat;\">" + Data[i].CName + "</b></span></a>";
                }
                if (Data[i].Split == '1') {
    ...
    function Toolbar_Children(ButtonId, Data) {
        ...
        $.each(Data, function (i) {
            if (Data[i].ParentID == ButtonId) {
                var title = "title=\"" + Data[i].Remark + "\"";
                if (Data[i].Remark == "") { title = ""; }
                _toolbar_Children += "<li id=\"" + Data[i].Code + "\" " + title + " onclick=\"" + Data[i].JsEvent + "\"><img src=\""+staticImg+"/images/Icon16/" + Data[i].Icon + "\" />" + Data[i].CName + "</li>";
    ...
    $.each(JsonData, function (i) {
        if (JsonData[i].ParentID == 0 && JsonData[i].Category == '2') {
            ...
            var righttoolbar_Children_List = right_toolbar_Children(JsonData[i].ID, JsonData)
            if (right_toolbar_Count > 0) {
                right_toolbar_html += "<li id=\"right_" + JsonData[i].Code + "\" " + title + " onclick=\"" + JsonData[i].JsEvent + "\" ><img src=\""+staticImg+"/images/Icon16/" + JsonData[i].Icon + "\" />" + JsonData[i].CName + righttoolbar_Children_List + "</li>";
            } else {
                right_toolbar_html += "<li id=\"right_" + JsonData[i].Code + "\" " + title + " onclick=\"" + JsonData[i].JsEvent + "\" ><img src=\""+staticImg+"/images/Icon16/" + JsonData[i].Icon + "\" />" + JsonData[i].CName + "</li>";
            }
    ...
    function right_toolbar_Children(ButtonId, JsonData) {
        ...
        $.each(JsonData, function (i) {
            if (JsonData[i].ParentId == ButtonId) {
                var title = "title=\"" + JsonData[i].Remark + "\"";
                if (JsonData[i].Remark == "") { title = ""; }
                _right_toolbar += "<li id=\"right_" + JsonData[i].Code + "\" " + title + " onclick=\"" + JsonData[i].JsEvent + "\" ><img src=\""+staticImg+"/images/Icon16/" + JsonData[i].Icon + "\" />" + JsonData[i].CName + "</li>";
                if (JsonData[i].Split == '1') {
...
//=================动态菜单tab标签========================
function AddTabMenu(tabid, url, name, img, Isclose, staticImg, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','true','" + staticImg + "')\"><a><img src='"+staticImg+"/images/Icon16/" + img + "' width='16' height='16'>" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','false','" + staticImg + "')\"><a><img src='"+staticImg+"/images/Icon16/" + img + "' width='16' height='16'>" + name + "</a></li>");
        }
...
function VisitorModule(ModuleId, ModuleName) {
    ...
    getAjax("/Home/SetModuleId.json", { ModuleId: ModuleId, ModuleName: ModuleName }, function (data) { })
}
//离开模块
function SetLeave(ModuleId, ModuleName) {
    getAjax("/Home/SetLeave.json", { ModuleId: ModuleId, ModuleName: ModuleName }, function (data) { })
}
//=================动态菜单tab标签========================
//js获取网站根路径(站点及虚拟目录)
function RootPath() {
...
function btn_Search(SearchField) {
    var SearckKey = $("#txtSearckKey").val();
    var postData = { SearchField: SearchField, SearckKey: SearckKey };
    var Parameter = [];
    $(".searchItem").each(function () {
        var searchitemname = $(this).data("searchitemname");
        var value = $(this).val();
        if (value && value != "") {
            Parameter.push({
                Name: searchitemname,
    ...
    postData["ParameterJson"] = JSON.stringify(Parameter);
    $("#gridTable").setGridParam({ postData: postData }).trigger("reloadGrid")
}