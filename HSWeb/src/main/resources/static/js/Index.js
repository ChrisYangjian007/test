...
/**安全退出**/
function IndexOut() {
    var msg = "<div class='ui_alert'>确认要退出 农产品质量追溯管控平台？</div>"
    top.$.dialog({
        id: "confirmDialog",
        lock: true,
        icon: "hits.png",
    ...
    AjaxJson("/Home/ShortcutsListJson", {}, function (DataJson) {
        ...
        if (DataJson.length>0) {
            $.each(DataJson, function (i) {
                $("#Shortcuts").append("<li onclick=\"AddTabMenu('" + DataJson[i].moduleID + "', '" + DataJson[i].location + "', '" + DataJson[i].cName + "',  '" + DataJson[i].icon + "','true');\"><img src=\"/images/Icon16/" + DataJson[i].icon + "\" />" + DataJson[i].cName + "</li>");
            });
        }
    });
    $(".popup li").click(function () {
